...
trait Register
{
	...
	public function register(UserRequest $request)
	{
		...
		// Save Company Data
		$isEmployerType = (config('larapen.core.register.showCompanyFields') && $request->input('user_type_id') == 1);
		if ($isEmployerType && isset($user->id)) {
			$this->storeCompany($user->id, $request);
		}
		
		// Save Candidate Data
		$isJobSeekerType = (config('larapen.core.register.showResumeFields') && $request->input('user_type_id') == 2);
		if ($isJobSeekerType && isset($user->id)) {
