...
class BlacklistController extends PanelController
{
	...
	/**
	 * Ban user (by email or/and phone number) (from link)
	 *
	...
	public function banUser(): \Illuminate\Http\RedirectResponse
	{
		...
		if (!empty($email)) {
			...
			if ($bannedEmail->exists()) {
				 ...
				 * that delete the user and its listings (if exist).
				 */
		...
		if (!empty($phone)) {
			...
			$bannedPhone = Blacklist::where('type', 'phone')->where('entry', $phone);
