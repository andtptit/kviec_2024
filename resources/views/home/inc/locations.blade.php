...
$sectionData ??= [];
$adminsDivisions = (array)data_get($sectionData, 'adminsDivisions');
											...
											@foreach ($items as $k => $city)
												<li>
													@if (data_get($city, 'id') == 0)
														<a href="#browseAdminCities" data-bs-toggle="modal">{!! data_get($city, 'name') !!}</a>
													@else
								...
								@else
									@if (in_array(auth()->user()->user_type_id, [1]))
										<a class="btn btn-lg btn-listing ps-4 pe-4" href="{{ \App\Helpers\UrlGen::addPost() }}" style="text-transform: none;">
											<i class="fa fa-plus-circle"></i> {{ t('create_new_job') }}
										</a>
									@endif
								@endif
							@endif
