					...
					@foreach ($countries as $item)
						<option value="{{ $item->get('code') }}" {{ (
									old(
										...
										(data_get($company, 'country_code') ?? ((!empty(config('country.code'))) ? config('country.code') : 0))
									)==$item->get('code')
								) ? 'selected="selected"' : ''
							}}
						>
...
@section('after_scripts')
	...
	<script>
		...
		/* Locations */
		var countryCode = '{{ old('company.country_code', (data_get($company, 'country_code') ?? 0)) }}';
		var adminType = 0;
		var selectedAdminCode = 0;
		var cityId = '{{ old('company.city_id', (data_get($company, 'city_id') ?? 0)) }}';
	</script>
